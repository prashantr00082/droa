...
logger = get_logger('MemoryUpdater')
import os
...
def extract_and_store_rule(repo_root: str, critique: str):
    logger.info("  [Memory Updater] Extracting rule from critique for Continual Learning...")
    ...
    try:
        ...
        logger.info(f"  [Memory Updater] Extracted Rule for [{target}]: {new_rule}")
        ...
        logger.info(f"  [Memory Updater] Successfully appended rule to local memory ({mem_path}).")
        
    except Exception as e:
        logger.error(f"  [Memory Updater] Failed to extract or store rule: {e}")
